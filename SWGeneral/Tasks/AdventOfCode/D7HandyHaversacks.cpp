...
D7HandyHaversacks::D7HandyHaversacks() {
	parseDataFile("D:\\GitProjects\\UltimadeMode\\SWGeneral\\Data\\stage_7_test.txt");
}
...
int D7HandyHaversacks::parseDataFile(std::string filePath) {
	...
	if(file.is_open()) {
		...
		std::regex inherited_value_pattern(R"((\d+) (\w+ \w+) ?)");
		...
		while(getline(file, currentString)) {
			...
			std::cout << root_match[1] << std::endl;
			for (std::sregex_iterator i = words_begin; i != words_end; ++i) {
				std::smatch match = *i;
				std::string match_str = match.str();
				std::cout << match_str[0] << "|" << match[2] << "|" << '\n';
			}
			std::cout << "---------------------------" << std::endl;
		}
